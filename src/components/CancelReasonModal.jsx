...
export default function CancelReasonModal({ open, onClose, onConfirm, orderId }) {
    ...
    return (
        <Modal open={open} onClose={handleClose} title={`Hủy đơn hàng${orderId ? ` — ${orderId}` : ''}`}>
            <div className="space-y-4">
                <div className="flex items-start gap-3 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3">
                    ...
                    <p className="text-sm text-red-300">
                        Thao tác này sẽ hủy đơn hàng và <strong className="text-red-200">hoàn trả toàn bộ số lượng tồn kho</strong>. Không thể hoàn tác.
                    </p>
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-400 mb-1.5">
                        Lý do hủy <span className="text-gray-600 font-normal">(không bắt buộc)</span>
                    </label>
                    <textarea
                        rows={3}
                        ...
                        placeholder="VD: Khách yêu cầu hủy, hết hàng…"
                        value={reason}
